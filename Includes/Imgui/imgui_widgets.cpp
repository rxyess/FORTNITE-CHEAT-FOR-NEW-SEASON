...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImColor bodyColor = ImColor(int(47), int(71), int(255), 255);
    ImColor inactiveColor = ImColor(int(37), int(42), int(63), 255);
    ImU32 check_col2 = bodyColor;
    ImU32 check_col3 = inactiveColor;
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? check_col2 : hovered ? check_col2 : check_col3), true, style.FrameRounding);
    ...
    else if (*v)
    {
        ...
        RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? check_col2 : hovered ? check_col2 : check_col2), true, style.FrameRounding);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        ...
        if (focus_requested || clicked || g.NavActivateId == id || g.NavActivateInputId == id)
        {
            ...
            FocusWindow(window);
            g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
            if (focus_requested || (clicked && g.IO.KeyCtrl) || g.NavActivateInputId == id)
            {
    ...
    ImColor bodyColor = ImColor(int(47), int(71), int(255), 255);
    // Render grab
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.GetCenter().y), ImVec2(frame_bb.Max.x - 10, frame_bb.GetCenter().y), frame_col, 2);
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, grab_bb.GetCenter().y), ImVec2(grab_bb.Max.x - 10, grab_bb.GetCenter().y), bodyColor, 2);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        window->DrawList->AddCircleFilled(ImVec2((grab_bb.Min.x + grab_bb.Max.x) / 2, grab_bb.GetCenter().y), 4, GetColorU32(frame_col));
    }
    //window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
